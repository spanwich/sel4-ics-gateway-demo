#!/bin/bash
# Demo launcher (ARM) - Creates tmux session with 4 quadrants
#
# Uses ARM seL4 gateway with QEMU emulation (cortex-a15)
#
# Layout:
# ┌─────────────────┬─────────────────┐
...
# Pane 1: seL4 Gateway (ARM)
tmux send-keys -t "$SESSION:0.1" "echo '=== seL4 Gateway - ARM (Port 502) ===' && echo 'Waiting for PLC...' && sleep 5 && sudo GATEWAY_ARCH=arm docker compose up gateway" C-m
...
ICS Gateway Demo (ARM) - CVE Attack Tools

ARCHITECTURE: ARM (QEMU emulation, cortex-a15)
